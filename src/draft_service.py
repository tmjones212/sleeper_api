...
class DraftService:
	...
	def get_ktc_player_value(self) -> List[Dict[str, Any]]:
		...
		# Use the KTC service to get player values
		from ktc_service import KTCService
		ktc_service = KTCService()
		return ktc_service.get_player_values()

	def get_draft_pick_ktc_ids(self) -> Dict[int, str]:
		"""Get KTC IDs and names for draft picks."""
		# Use the KTC service to get draft pick IDs
		from ktc_service import KTCService
		ktc_service = KTCService()
		return ktc_service.get_draft_pick_ids()
